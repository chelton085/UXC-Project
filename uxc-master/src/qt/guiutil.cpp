...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("ultraxcoin"))
        return false;
...
{
    // Convert ultraxcoin:// to ultraxcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ultraxcoin://"))
    {
        uri.replace(0, 12, "ultraxcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ultraxcoin.lnk";
}
...
{
    return GetAutostartDir() / "ultraxcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ultraxcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ultraxcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ultraxcoin-Qt"));
